...
$selsql = mark_sql_post("SELECT id FROM Likes WHERE user = [currentuser] AND post = [postid]");
$result = $conn->query($selsql);

if($rs = $result->fetch_array(MYSQLI_ASSOC)) {
    $delsql = mark_sql_post("DELETE FROM Likes WHERE user = [currentuser] AND post = [postid]");
    $result = $conn->query($delsql);    
} else {
    $insertsql = mark_sql_post("INSERT INTO Likes(user, post, like_dislike) VALUES ([currentuser], [postid], '1')");
